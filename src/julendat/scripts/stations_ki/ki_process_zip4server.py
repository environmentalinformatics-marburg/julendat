...
def move_files(source_path, target_path):
    ...
    cmd = 'move ' + source_path + ' ' + target_path
    os.system(cmd)
    return target_path

def copy_files(source_path, target_path):
    '''Copy files - just in case.

    Args:
        source_path: Path from which to copy
        target_path: Path to copy
    '''
    print "Source: ", source_path
    print "Target: ", target_path
    if not os.path.exists(target_path): os.makedirs(target_path)
    cmd = 'copy ' + source_path + ' ' + target_path
    os.system(cmd)
    return target_path
...
def zip_files(source_path, target_path):
    ...
    '''
    if ".7z" not in target_path:
        target_path = target_path + os.sep + \
                    datetime.now().strftime('%Y%m%d-%H%M%S') + '.7z' 
    if ".ods" in source_path:
        cmd = '7z a ' + target_path + ' ' + source_path
    else:
...
def configure(config_file):
    ...
    toplevel_zip_path = config.get('zip4server', 'toplevel_zip_path')
    taskbook_path = config.get('zip4server', 'taskbook_path')
    return toplevel_processing_plots_path, project_id, toplevel_temp_path, toplevel_zip_path, taskbook_path
...
def main():
    ...
    toplevel_processing_plots_path, project_id, toplevel_temp_path , toplevel_zip_path, taskbook= \
    ...
    print "Adding taskbook ", 
    act_file = zip_files(taskbook, act_file)
    print "Moving files "
    target_path = toplevel_temp_path + datetime.now().strftime('%Y%m%d-%H%M%S')
    target_path = copy_files(act_file, target_path)
    target_path = move_files(act_file, toplevel_zip_path)
    print "...finished"
